...
// READ IN VARIABLES
tpw_car_version = "1.37"; // Version string
...
tpw_car_fnc_carspawn =
	...
	_driver forceAddUniform (_clothes select  (floor (random (count _clothes)))); 
	...
	if (random 10 < 5) then 
		...
		_passenger forceAddUniform (_clothes select  (floor (random (count _clothes)))); 
