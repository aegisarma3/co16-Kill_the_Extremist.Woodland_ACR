...
// READ IN VARIABLES
tpw_civ_version = "1.37"; // Version string
...
// GET APPROPRIATE CIVS AND CLOTHES
_civlist =tpw_core_civs;
_clothes=tpw_core_clothes;
...
tpw_civ_fnc_civspawn =
	...
	// Random clothes
	_civ forceAddUniform (_clothes select  (floor (random (count _clothes))));
	...
	if (tpw_civ_active) then
		...
		for "_i" from 0 to (count tpw_civ_civarray - 1) do
			...
			if !(alive _civ) then 
				...
				{
				// Set fatigue so civs walk more slowly
				if (speedmode _civ == "LIMITED" && {stance _civ == "STAND"}) then
					{
					_civ setfatigue 1;
					} else
					{
